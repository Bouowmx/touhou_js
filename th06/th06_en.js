...
var player_images = [];
for (i = 0; i <= 3; i++) {
	player_images[i] = new Image();
	player_images[i].src = "player/reimu_" + i + ".png";
}
var player_sprite = 0;
var shoot = 1;
var time_g = $.now();
...
var frame = function() {
	/*var player = $("#player")[0];
	player.src = "player/reimu_" + player_sprite++ + ".png";
	if (player_sprite > 3) {player_sprite = 0;}
	...
	if (draw_player == 1) {
		context_2d.drawImage(player_images[player_sprite++], 300, 400);
		if (player_sprite > 3) {player_sprite = 0;}
		draw_player = 1;
	} else {draw_player--;}
	/*var time = $.now();
